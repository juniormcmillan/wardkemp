...
if($listings["type"] == "N")
{
	$strSQL = "SELECT * FROM user_access WHERE active = 0 ORDER BY date_created, surname, forename";
	$pageTitle = "New Users";
}
elseif($listings["type"] == "A")
{
	$strSQL = "SELECT * FROM user_access WHERE active = 1 ORDER BY date_created, surname, forename";
	$pageTitle = "Active Users";
}
elseif($listings["type"] == "P")
{
	$strSQL = "SELECT * FROM user_access WHERE partner = 'Y' ORDER BY surname, forename";
	$pageTitle = "Partners";
}
elseif($listings["criteria"] != "")
{
	$strSQL = "SELECT * FROM user_access WHERE (LOWER(forename) = '".$listings["criteria"]."' OR LOWER(surname) = '".$listings["criteria"]."' OR LOWER(company_name) = '".$listings["criteria"]."') ORDER BY surname, forename";

	$criteria	=	$listings["criteria"];

	$strSQL = "SELECT * FROM user_access WHERE (forename like '%$criteria%' OR surname like '%$criteria%'  OR company_name like '%$criteria%' ) ORDER BY surname, forename";

	$pageTitle = "Search Results";
}
else
{
	$strSQL = "SELECT * FROM user_access ORDER BY surname, forename";
	$pageTitle = "All Users";
}
//echo $strSQL;exit();
