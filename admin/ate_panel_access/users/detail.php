	...
	if($formErrors == "")
	{
		...
		else
		{
			...
			if($tmpActive == 1 && $tmpActive != $tmpOldActive)
			{
				...
				Your Box Legal Client Area account has been activated.
				...
				Login Here: http://www.boxlegal.co.uk/
				...
				*********************************************
				...
				# active
				$company_id						=	$gMysql->QueryItem("SELECT company_id FROM user_access where id='$tmpId'",__FILE__,__LINE__);
				$applyIndividualPremiumPoints	=	$gMysql->QueryItem("SELECT applyIndividualPremiumPoints FROM solicitor where code='$company_id'",__FILE__,__LINE__);
